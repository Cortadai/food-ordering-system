...
public class PaymentOutboxScheduler implements OutboxScheduler {
    ...
    public void processOutboxMessage() {
        Optional<List<OrderPaymentOutboxMessage>> orderMessagesResponse =
        ...
        if (orderMessagesResponse.isPresent() && orderMessagesResponse.get().size() > 0) {
            List<OrderPaymentOutboxMessage> outboxMessages = orderMessagesResponse.get();
            log.info("Received {} OrderPaymentOutboxMessages with ids: {}, sending to message bus!",
                    outboxMessages.size(),
            ...
            outboxMessages.forEach(outboxMessage ->
                paymentRequestMessagePublisher.publish(outboxMessage,
                        (orderPaymentOutboxMessage, outboxStatus) -> updateOutboxStatus(
